  ...
  layers.forEach(function(layer, i) {
    map.addLayer({
      ...
      paint: {
        ...
        "fill-opacity": 0.6,
        "fill-extrude-height": {
          //grab the height from the 'EleZ' attribute from building data
          type: "identity",
          property: "EleZ"
        },
        "fill-extrude-base": 0
...
map.on("mousemove", function(e) {
  ...
  if (features.length > 0) {
    ...
    map.getCanvas().style.cursor =
      features[0].properties.EleZ != null ? "pointer" : "";
  } else {
...
map.on("click", function(e) {
  ...
  var features = map.queryRenderedFeatures(e.point, {
    layers: all_added_layers
  });
  if (features.length > 0 && features[0].properties.EleZ != null) {
    ...
    popup.setHTML(
      "<center>The height of this building is approximately: <b>" +
        Math.round(features[0].properties.EleZ) +
        "</b> meters."
    );
